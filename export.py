...
from pre_models.experimental import attempt_load, End2End
from pre_models.yolo import ClassificationModel, Detect, DDetect, DualDetect, DualDDetect, DetectionModel, SegmentationModel
...
def export_saved_model(model,
    ...
    from pre_models.tf import TFModel
...
def export_tflite(keras_model, im, file, int8, data, nms, agnostic_nms, prefix=colorstr('TensorFlow Lite:')):
    ...
    if int8:
        from pre_models.tf import representative_dataset_gen
...
def add_tflite_metadata(file, metadata, num_outputs):
    # Add metadata to *.tflite pre_models per https://www.tensorflow.org/lite/models/convert/metadata
...
def run(
    ...
    if any((saved_model, pb, tflite, edgetpu, tfjs)):  # TensorFlow formats
        assert not tflite or not tfjs, 'TFLite and TF.js pre_models must be exported separately, please pass only one type.'
